...
def test():
    assert mul_sum('xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))') == 161
...
def main():
    ...
    with open(file) as f:
        s = f.read().strip()
    print(mul_sum(s))
    test()  # No need to print the result of test()
...
def mul_sum2(s):
    ...
    while index < len(s):
        ...
        if s[index:index + 6] == "don't(":
            enabled = False
            index += 6
        # Check for "do()" instruction
        elif s[index:index + 3] == "do(":
            enabled = True
            index += 3
        # Check for valid mul() instruction
        else:
            match = re.match(r'mul\((\d+),(\d+)\)', s[index:])
            if match:
                if enabled:
                    x, y = match.groups()
                    mul += int(x) * int(y)
